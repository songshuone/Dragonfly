...
func (p2p *P2PDownloader) getItem(latestItem *Piece) (bool, *Piece) {
	...
	p2p.Ctx.ClientLogger.Warnf("Piece latestItem=========> %s", latestItem.String())
...
func (p2p *P2PDownloader) processPiece(response *types.PullPieceTaskResponse,
	...
	p2p.refresh(item)
	p2p.Ctx.ClientLogger.Infof("response=========>%s", response.String())
	p2p.Ctx.ClientLogger.Infof("item item=========>%s", item.String())
